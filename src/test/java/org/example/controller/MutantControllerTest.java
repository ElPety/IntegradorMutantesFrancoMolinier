...
class MutantControllerTest {
    ...
    // Faltaba inyectar esto para poder usar "objectMapper.writeValueAsString"
    @Autowired
    ...
    void testCheckMutantReturns200ForMutant() throws Exception {
        ...
        // ACT & ASSERT
        mockMvc.perform(
                        post("/mutant") // Requiere la importación estática de MockMvcRequestBuilders
                                .contentType(MediaType.APPLICATION_JSON)
                                .content(objectMapper.writeValueAsString(request))
                )
                .andExpect(status().isOk());  // Requiere la importación estática de MockMvcResultMatchers
    }
